...
export const Metronome: React.FC = () => {
    ...
    const toggle = async () => {
        ...
        if (isPlaying) {
            ...
            try {
                // 3. Request Mic Permission (Force PlayAndRecord) & Wait for Mode Switch
                // We use raw getUserMedia here to ensure we block until permission is granted
                // and the OS has time to switch audio modes (e.g. to SCO).
                // startAnalysis() from hook might yield early if context is not ready in closure.
                console.log('[Toggle] Requesting raw mic access to force mode switch...');
                const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
                // We don't need this stream, just the mode switch. Release it.
                stream.getTracks().forEach(t => t.stop());

                // Now that permission is granted and mode presumably switched...
